...
func (c *Checker) checkBranchProtection(repos []*gh.Repository) []engine.Finding {
	...
	if len(unprotected) == 0 {
		return []engine.Finding{pass("github_branch_protection", "All repos have branch protection with required reviews", soc2("CC8.1"))}
	}
...
func (c *Checker) checkPublicRepos(repos []*gh.Repository) []engine.Finding {
	...
	if len(public) == 0 {
		return []engine.Finding{pass("github_public_repos", "No unexpected public repositories", soc2("CC6.6"))}
	}
	return []engine.Finding{fail(
		"github_public_repos",
...
func (c *Checker) checkSecretScanning(repos []*gh.Repository) []engine.Finding {
	...
	for _, repo := range repos {
		if repo.GetArchived() || repo.GetPrivate() {
			// secret scanning on private repos requires GitHub Advanced Security
			continue
		}
	...
	if len(disabled) == 0 {
		return []engine.Finding{pass("github_secret_scanning", "Secret scanning enabled on public repos", soc2("CC6.8"))}
	}
	return []engine.Finding{fail(
